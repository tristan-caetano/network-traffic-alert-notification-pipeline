# Powershell script that is run from pcap_to_csv.py if the GUI is being run on windows    
    ...
    & 'C:\Program Files\Wireshark\tshark.exe' -r $wInput -T fields -E separator="," -e ip.src -e tcp.srcport -e ip.dst -e tcp.dstport -e _ws.col.Protocol -e frame.time_relative -e tcp.analysis.bytes_in_flight -e ip.ttl -e tcp.seq -e tcp.ack > $wOutput